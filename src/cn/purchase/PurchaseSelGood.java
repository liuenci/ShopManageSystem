 ...
 *
 */
public class PurchaseSelGood extends JPanel {
	...
	public PurchaseSelGood(JSplitPane splittopSelGood) {
		this.splittopbottom = splittopSelGood;
	...
	public void setTopPanel() {
		...
		// 布局为null
		ptop.setLayout(null);
		JLabel lbTitle = comMenth.createLabel("订单审核", 340, 10, 180, 40, "隶书", 30);
		...
		tabletop.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					...
					if (getrow > -1) {
						...
						if (res == JOptionPane.YES_OPTION) {
							...
							for (int j = 0; j < rowbottom; j++) {
								...
								// 商品信息
								afrow = goodpurd.updateSelGood((int) tabletop.getValueAt(getrow, 0),
										(int) tablebottom.getValueAt(j, 0));
								// 修改table的信息
								tmtop.setValueAt("取消订单", getrow, 5);
								tmbottom.setValueAt("已退货", j, 11);
							}
						} else {
							// 错误操作警告语句
					...
					topRow = tabletop.getSelectedRow();
					int column = tabletop.getAutoResizeMode();
					tabletop.getValueAt(topRow, column);
					purid = (int) tabletop.getValueAt(topRow, 0);
					if (topRow != -1) {
						// 创建表格
						String[] str = { "商品编号", "商品名称", "单位", "规格型号", "进价", "数量", "总价", "售价", "仓库编号", "保质期", "最低库存",
								"订单状态" };
						PurDetail pur = new PurDetail();

						// 获取数据库数据
						PurchaseOrderDetailGoodDao1 goodpurd = new PurchaseOrderDetailGoodDao1();
						// 商品信息
						List<PurchaseOrderDetailGood> lstugood = goodpurd.queryId(purid);
						// 商品信息二维数组
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					...
					if (row != -1) {
						...
						// 选择提示语句
						int res = JOptionPane.showOptionDialog(null, "是否确定删除！", "操作提示",
								JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, obj, obj[0]);
						if (res == JOptionPane.YES_OPTION) {
							// 创建表格
							String[] str = { "商品编号", "商品名称", "单位", "规格型号", "进价", "数量", "总价", "售价", "仓库编号", "保质期",
									"最低库存", "订单状态" };
							// 获取数据库数据
							PurchaseOrderDetailGoodDao1 goodpurd = new PurchaseOrderDetailGoodDao1();
							// 商品信息
							goodpurd.updateSelGoods(purid, goodid);
							// 删除该行
							tmbottom.removeRow(row);
							tmtop.setValueAt("未审核", topRow, 5);
							// 判断是否为最后一件商品
							if (tmbottom.getRowCount() == 0) {
								goodpurd.updatePassPur(purid);
								tmtop.setValueAt("取消订单", topRow, 5);
							}
						} else {
							// 退货失败提示
		...
		btnSelTime.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!tfgoodTime.getText().equals("") || !tfgoodTime.getText().equals("单击选择时间")) {
					...
					PurchaseOrderDetailGoodDao1 goodpurd = new PurchaseOrderDetailGoodDao1();
					// 订单信息
					List<PurchaseOrderEmpSup> lstu = goodpurd.queryOrdTime(pur_date);
					// 订单信息二维数组
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!tfgoodid.getText().equals("")) {
					try {
						...
						PurchaseOrderDetailGoodDao1 goodpurd = new PurchaseOrderDetailGoodDao1();
						// 订单信息
						List<PurchaseOrderEmpSup> lstu = goodpurd.queryOrdId(pur_id);
						// 订单信息二维数组
						Object[][] rowtop = new Object[lstu.size()][];
						for (int i = 0; i < rowtop.length; i++) {
							// 获取集合对象
							PurchaseOrderEmpSup purempsup = lstu.get(i);
							String purstatus = purempsup.getPur_status() == 0 ? "未审核"
									: (purempsup.getPur_status() == 1 ? "审核通过" : "审核未通过");
							// 将对象属性转为数组存储
							Object[] obj = { purempsup.getPur_id(), purempsup.getSup_name(), purempsup.getPur_date(),
		...
		btnAllDel.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (getrow > -1) {
					Object[] obj = { "任性取消", "手下留情" };
					// 选择提示语句
					int res = JOptionPane.showOptionDialog(null, "是否取消订单！", "操作提示", JOptionPane.YES_NO_CANCEL_OPTION,
					...
					if (res == JOptionPane.YES_OPTION) {
						...
						for (int j = 0; j < rowbottom; j++) {
							...
							if (afrow > 0) {
								...
								// 错误操作警告语句
								JOptionPane.showMessageDialog(null, "退货操作失败！", "操作提示", JOptionPane.ERROR_MESSAGE);
							}
						}
					} else {
						// 错误操作警告语句
		...
		btnLitDel.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (row != -1) {
					...
					Object[] obj = { "任性删除", "手下留情" };
					// 选择提示语句
					int res = JOptionPane.showOptionDialog(null, "是否确定删除！", "操作提示", JOptionPane.YES_NO_CANCEL_OPTION,
							JOptionPane.QUESTION_MESSAGE, null, obj, obj[0]);
					if (res == JOptionPane.YES_OPTION) {
						// 创建表格
						String[] str = { "商品编号", "商品名称", "单位", "规格型号", "进价", "数量", "总价", "售价", "仓库编号", "保质期", "最低库存",
						...
						PurchaseOrderDetailGoodDao1 goodpurd = new PurchaseOrderDetailGoodDao1();
						// 商品信息
						afrow=goodpurd.updateSelGoods(purid, goodid);
						if (afrow > 0) {
							JOptionPane.showMessageDialog(null, "退货成功！");
							// 删除该行
							tmbottom.removeRow(row);
